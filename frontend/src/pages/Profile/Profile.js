...
const Profile = () => {
	...
	useEffect(() => {
		...
		axios
			...
			// .get(`http://localhost:5001/post/view/${props.userId}`)
			.then(function (response) {
				dispatch(createPost(response.data));
			});
		axios
			.get(`http://localhost:5001/liveupdate/view/${dataProfile._id}`)
			// .get(`http://localhost:5001/liveupdate/view/${props.userId}`)
			.then(function (response) {
			...
			<div className="profile_container">
				<div className="profile_pic_and_userName">
					...
					<div className="userName_in_profile_page">
						<h1 className="userName">{dataProfile?.firstname} {dataProfile?.lastname}</h1>
						...
						<div className="instagram_img_and_userName">
							...
							<span className="insta_userName">{dataProfile?.socialMediaHandle}</span>
						</div>
						<div className="bio">
